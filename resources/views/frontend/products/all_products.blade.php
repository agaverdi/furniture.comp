...
@section('title',strtoupper($category->category_name))
@section('styles')
...
@section('content')
    ...
    <main id="mt-main">
        ...
        <section class="mt-contact-banner style4 wow fadeInUp" data-wow-delay="0.4s" style="background-image: url({{asset($category->image)}}); background-repeat: no-repeat;background-size: inherit;">
            <div class="container">
                <div class="row">
                                    ...
                                    @foreach($products_for_banner as $product)
                                        @if($product->product_details->contains('hot_sale_items', 1) && $loop->iteration <= 4)
                                            ...
                                            <div class="mt-product4 mt-paddingbottom20">
                                                <div class="img">
                                                    <a href="{{ route('frontend.product.index', ['level1'=>$category->slug, 'level2'=>$product->subCategory->slug, 'level3'=>$product->slug]) }}"><img alt="image description" src="{{ asset($product->path1) }}"></a>
                                                </div>
                                                <div class="text">
                                                    <div class="frame">
                                                        <strong><a href="{{ route('frontend.product.index', ['level1'=>$category->slug, 'level2'=>$product->subCategory->slug, 'level3'=>$product->slug]) }}">{{ $product->name }}</a></strong>
                                                        <ul class="mt-stars">
                                                            @for($i=1;$i<=5;$i++)
                        ...
                        <div class="btn-box">
                            <ul class="list-inline">
                                <li>
                                    ...
                                    <div class="drop">
                                        <ul class="list-unstyled">
                                            <li><a class="ajax-link" href="#" data-url="/product/all_sort" data-sort-option="asc">A - Z ardicilliqi</a></li>
                                            <li><a class="ajax-link" href="#" data-url="/product/all_sort" data-sort-option="dsc">Z - A ardicilliqi</a></li>
                                            <li><a class="ajax-link" href="#" data-url="/product/all_sort" data-sort-option="price-asc">qiymete gore ucuz</a></li>
                                            <li><a class="ajax-link" href="#" data-url="/product/all_sort" data-sort-option="price-dsc">qiymete gore bahali</a></li>
                                        </ul>
                                    </div>
                                </li>
                            </ul>
                                ...
                                <div class="product-post">
                                    ...
                                    <div class="txt-holder">
                                        <div class="align-left">
                                            <strong class="title"><a href="{{ route('frontend.product.index', ['level1'=>$category->slug,'level2'=>$product->subCategory->slug,'level3'=>$product->slug,]) }}">{{ $product->name }}</a></strong>
...
@section('scripts')
    ...
    <script>
        ...
        function sendData(fromValue,toValue,product_category_id){
            var csrfToken = $('meta[name="csrf-token"]').attr('content');
            console.log(product_category_id);
            $.ajax({
                url: '/all_product/range',
                method: "POST",
                headers: {
                ...
                success:function (response){
                    if (response.status === 200) {
                        ...
                        $.each(response.products.data,function (index, product){
                            console.log(product)
                            var productDescription = product.description ? product.description.substring(0, 220) : '';
                            var html = `
                            <div class="product-post">
                                    <div class="img-holder">
                                        <img src="{{ asset('${product.path1}') }}" alt="image description">
                                    ...
                                    <ul class="list-inline">
                                        ${Array.from({ length: response.lastPage }, (_, i) => `
                                            <li class="${i + 1 === response.currentPage ? 'active' : ''}">
                                                <a href="${response.category_name}?page=${i + 1}" data-page="${i + 1}">${i + 1}</a>
                                            </li>
                                        `).join('')}
                                    </ul>
                                </nav>`;
        ...
        $("#sendDataBtn").on('click', function (){
            ...
            sendData(fromValue, toValue,product_category_id);
        });
                ...
                success: function (data) {
                    ...
                    if (data.products.data.length === 0) {
                        ...
                        $.each(data.products.data, function (index, value) {
                            ...
                            // Check if description exists and is not undefined before using substring
                            var productDescription = value.description.substring(0, 220)==null ? value.description.substring(0, 220) : 'daha artiq melumat ucun daxil olun';
                            console.log(value.description.substring(0, 220))
                            var html = `
                                <div class="product-post">
                                    <div class="img-holder">
