                ...
                <div class="row mb-2">
                    ...
                    <div class="col-sm-6">
                        <ol class="breadcrumb float-sm-right">
                            ...
                            <li class="breadcrumb-item active">Set Products</li>
                        </ol>
                    </div>
                </div>
            </div><!-- /.container-fluid -->
                                ...
                                <table id="example1" class="table table-bordered table-striped">
                                    ...
                                    @foreach($set_products as $set)
                                        <tr>
                                            ...
                                            <td>{{ $set->set_slug }}</td>
                                            <td>{{ $set->set_price }}</td>
                                            <td>{{ $set->set_discount }}</td>
                                            <td>{{ $set->created_at }}</td>
                                            <td>{{ $set->updated_at }}</td>
                                            <td>
                                                @if(auth('admin')->user()->permissions->update==3)
                                                <a class="btn btn-warning" href="{{ route('backend.set_product.edit', [$set->set_slug]) }}" ><i class="fas fa-edit">&nbsp;Edit</i></a>&nbsp;&nbsp;
                                                @endif
                                                @if(auth('admin')->user()->permissions->delete==4)
                                                <form action="{{ route('backend.set_product.destroy' ,$set->set_slug) }}" method="POST">
                                                    @csrf
                                                    @method('DELETE')
